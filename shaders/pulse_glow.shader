...
uniform float width : hint_range(0.0, 64) = 20.0;
uniform vec4 outline_color : hint_color = vec4(1.0, 1.0, 1.0, 0.1);
uniform int pixel_size : hint_range(1, 10) = 1;
uniform float width_speed :hint_range(0.1, 10) = 2;

void fragment()
{
    float _width = width + ((cos(TIME*width_speed) + 1f) -2f) * 10f;
